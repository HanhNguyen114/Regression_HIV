...
nboot <- 100
set.seed(seed)
train.id <- dimnames(train.lrna2)[[1]]
boot.id <- replicate(nboot,sample(train.id, replace = TRUE))
boot.train <- list()
for (i in 1:nboot) {
  boot.train[[i]] <- train.lrna2[boot.id[,i],]
}
#########################################################################################
rpruned <- list()
subn <- 30
...
mean.train.error #1.343354 subn= 35, #1.342097 subn = 30
#########test error
# test error
mean.mspe.lrna2.svm <- mean((test.lrna2$y - pred.svm.test)^2)
# 1.490711 NO NORM
sigma.mspe.lrna2.svm <- 1/ntest * var((test.lrna2$y - pred.svm.test)^2) 
...
#[1] 0.9311267 1.1037304 1.2763342 nboot=100 subn=35 > subn = 40
#0.9130087 1.0810834 1.2491582 subn = 30
# [1] 0.8321704 1.0896342 1.3470979 SUBN=30 NOT NORM

plot(pred.svm.test,test.lrna2$y)
testlm <- lm(test.lrna2$y~ pred.svm.test - 1)
abline(a = 0, b= 1)
abline(a = 0, b = testlm$coefficients[1])


var(test.lrna2$y) #[1] 1.044508
ggplot(data.frame(cbind(pred.svm.test,test.lrna2$y)), aes(x = pred.svm.test, y = test.lrna2$y))+
  geom_point() +geom_abline(intercept = 0, slope = 1) + 
  labs(title = 'lrna2 in test set vs predicted value - RF-SVM',x = 'Predicted value',y='True test value') 
cor(pred.svm.test,test.lrna2$y) 
# subn = 35, 0.0548309
# sub 30 = 0.08957727
#NOTNORM 30: 0.04689836

R2.rpart <- 1 - sum((test.lrna2$y-pred.svm.test)^2)/sum((test.lrna2$y-mean(test.lrna2$y))^2)
R2.rpart #[1] -0.05960916 subn=35 # subn 30:-0.03786748 #NOTNORM 30: -0.04123147
##########################################

...
###########################################################################################
# data for lrna2
data.lrna2 <- total_data.processed2[, !colnames(total_data.processed2) %in% c('cd42','lrna1','cd41')]
...
library(glmnet)
nvar <- ncol(train.lrna2)-1

# maintain the same folds across all models
fold_id <- sample(1:10, size = length(train.lrna2$y), replace=TRUE)
...
xmat.train.lrna2 <- model.matrix(~ -1 + ., train.lrna2[,colnames(train.lrna2) != 'y'])
xmat.test.lrna2 <- model.matrix(~ -1 + ., test.lrna2[,colnames(test.lrna2) != 'y'])
for(i in seq_along(tuning_grid$alpha)) {
  
  # fit CV model for each alpha value
  fit <- cv.glmnet(xmat.train.lrna2, train.lrna2$y, 
...
cv_net <- cv.glmnet(xmat.train.lrna2, 
                    train.lrna2$y, alpha = best_alpha)
...
lrna2.pred.glmnet <- predict(cv_net, s = cv_net$lambda.min, type="response",xmat.test.lrna2)
...
mean.mspe.lrna2 <- mean((test.lrna2$y - lrna2.pred.glmnet)^2)
sigma.mspe.lrna2 <- 1/ntest * var((test.lrna2$y - lrna2.pred.glmnet)^2) 
# 95% CI:
c(mean.mspe.lrna2 - 1.96 * sqrt(sigma.mspe.lrna2), mean.mspe.lrna2, mean.mspe.lrna2 + 1.96 * sqrt(sigma.mspe.lrna2))
# [1] 0.8251959 1.0736584 1.3221209
plot(lrna2.pred.glmnet,test.lrna2$y)
abline(a= 0, b= 1)

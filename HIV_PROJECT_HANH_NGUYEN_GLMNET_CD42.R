...
###########################################################################################
# data for cd42
data.cd42 <- total_data.processed2[, !colnames(total_data.processed2) %in% c('lrna2','lrna1','cd41')]
names(data.cd42)[3] <- 'y'
...
library(glmnet)
nvar <- ncol(train.cd42)-1

# maintain the same folds across all models
fold_id <- sample(1:10, size = length(train.cd42$y), replace=TRUE)
...
xmat.train.cd42 <- model.matrix(~ -1 + ., train.cd42[,colnames(train.cd42) != 'y'])
xmat.test.cd42 <- model.matrix(~ -1 + ., test.cd42[,colnames(test.cd42) != 'y'])
for(i in seq_along(tuning_grid$alpha)) {
  
  # fit CV model for each alpha value
  fit <- cv.glmnet(xmat.train.cd42, train.cd42$y, 
...
cv_net <- cv.glmnet(xmat.train.cd42, 
                    train.cd42$y, alpha = best_alpha)
...
cd42.pred.glmnet <- predict(cv_net, s = cv_net$lambda.min, type="response",xmat.test.cd42)
...
mean.mspe.cd42 <- mean((test.cd42$y - cd42.pred.glmnet)^2)
sigma.mspe.cd42 <- 1/ntest * var((test.cd42$y - cd42.pred.glmnet)^2) 
# 95% CI:
c(mean.mspe.cd42 - 1.96 * sqrt(sigma.mspe.cd42), mean.mspe.cd42, mean.mspe.cd42 + 1.96 * sqrt(sigma.mspe.cd42))
# [1] 0.3736965 0.5098506 0.6460046
plot(cd42.pred.glmnet,test.cd42$y)
abline(a= 0, b= 1)

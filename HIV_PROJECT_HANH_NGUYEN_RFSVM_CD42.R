...
nboot <- 100
set.seed(seed)
train.id <- dimnames(train.cd42)[[1]]
boot.id <- replicate(nboot,sample(train.id, replace = TRUE))
boot.train <- list()
for (i in 1:nboot) {
  boot.train[[i]] <- train.cd42[boot.id[,i],]
}
#########################################################################################
rpruned <- list()
subn <- 50
...
mean.train.error #subn=50  1.926921 not norm
#########test error
# test error
...
#subn=50 0.3490716 0.4740636 0.5990556 not norm
...
var(test.cd42$y) #[1] 1.044508
ggplot(data.frame(cbind(pred.svm.test,test.cd42$y)), aes(x = pred.svm.test, y = test.cd42$y))+
  geom_point() +geom_abline(intercept = 0, slope = 1) + 
  labs(title = 'cd42 in test set vs predicted value - RF-SVM',x = 'Predicted value',y='True test value') 
cor(pred.svm.test,test.cd42$y) 
#subn=50 0.7015884
# not norm 0.7013223
R2.rpart <- 1 - sum((test.cd42$y-pred.svm.test)^2)/sum((test.cd42$y-mean(test.cd42$y))^2)
R2.rpart #subn=50 0.4888409 #not norm 0.488313
##########################################

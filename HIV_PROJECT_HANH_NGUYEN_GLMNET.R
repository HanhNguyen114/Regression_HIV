...
corr <- subset(corr, abs(Freq) > 0.75) 
...
###########################################################################################
# data for lrna1
data.lrna1 <- total_data.processed2[, !colnames(total_data.processed2) %in% c('lrna2','cd41','cd42')]
...
library(glmnet)
nvar <- ncol(train.lrna1)-1

# maintain the same folds across all models
fold_id <- sample(1:10, size = length(train.lrna1$y), replace=TRUE)
...
xmat.train.lrna1 <- model.matrix(~ -1 + ., train.lrna1[,colnames(train.lrna1) != 'y'])
xmat.test.lrna1 <- model.matrix(~ -1 + ., test.lrna1[,colnames(test.lrna1) != 'y'])
for(i in seq_along(tuning_grid$alpha)) {
  
  # fit CV model for each alpha value
  fit <- cv.glmnet(xmat.train.lrna1, train.lrna1$y, 
...
cv_net <- cv.glmnet(xmat.train.lrna1, 
                    train.lrna1$y, alpha = best_alpha)
...
lrna1.pred.glmnet <- predict(cv_net, s = cv_net$lambda.min, type="response",xmat.test.lrna1)
...
mean.mspe.lrna1 <- mean((test.lrna1$y - lrna1.pred.glmnet)^2)
sigma.mspe.lrna1 <- 1/ntest * var((test.lrna1$y - lrna1.pred.glmnet)^2) 
# 95% CI:
c(mean.mspe.lrna1 - 1.96 * sqrt(sigma.mspe.lrna1), mean.mspe.lrna1, mean.mspe.lrna1 + 1.96 * sqrt(sigma.mspe.lrna1))

plot(lrna1.pred.glmnet,test.lrna1$y)
abline(a= 0, b= 1)

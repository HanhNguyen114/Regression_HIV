...
comat <- cor(data.lrna2[, !colnames(data.lrna2) %in% c('arm')])
...
set.seed(seed)
library(caret)
strat_index <- createDataPartition(y = data.lrna2$arm,list = FALSE,p = 0.75)
...
rpfit <- rpart(y ~., method="anova", control=parameter, data = train.lrna2)
table(rpfit$where)
...
cl <- list()
for (i in 1:k) {
  cl[[i]] <- train.lrna2[rpruned$where == leaf.id[i],]
}
...
#i = 1
#pred.tree <- predict(rpruned,newdata = test.lrna2[i,])
#whichleaf <- which(leaf.id == which(rpruned$frame$yval == pred.tree))
#pred.svm <- predict(svmfit[[whichleaf]],test.lrna2[i,])
...
pred.svm.vec <-NULL
for (i in 1:nrow(test.lrna2)){
pred.svm.vec[i] <- find.svmprediction(test.lrna2[i,])
}

#########train error
ntrain <- nrow(train.lrna2); ntest <- nrow(test.lrna2)
pred.svm.train.vec <- NULL; train.lrna2.y <- NULL
for (i in 1:length(cl)) {
  pred.svm.train.vec <- c(pred.svm.train.vec,pred.svm.train[[i]])
  train.lrna2.y <- c(train.lrna2.y,cl[[i]]$y)
}
# psi is the mean squared prediction error (MSPE) estimate
# sigma2 is the estimate of the variance of the MSPE
mean.mspe.lrna2.svm <- mean((train.lrna2.y - pred.svm.train.vec)^2)
sigma.mspe.lrna2.svm <- 1/ntrain * var((train.lrna2.y - pred.svm.train.vec)^2) 
# 95% CI:
...
#trainlm <- lm(train.lrna2.y~ pred.svm.train.vec - 1)
ggplot(data.frame(cbind(pred.svm.train.vec,train.lrna2$y)), aes(x = pred.svm.train.vec, y = train.lrna2$y))+
  geom_point() +geom_abline(intercept = 0, slope = 1)+
  labs(title = 'lrna2 in train set vs predicted value - RPART-SVM',x = 'Predicted value',y='True test value')
#0.05444087 0.11073697 0.16703307 not norm
#########test error
# test error
mean.mspe.lrna2.svm <- mean((test.lrna2$y - pred.svm.vec)^2)
sigma.mspe.lrna2.svm <- 1/ntest * var((test.lrna2$y - pred.svm.vec)^2) 
c(mean.mspe.lrna2.svm - 1.96 * sqrt(sigma.mspe.lrna2.svm), mean.mspe.lrna2.svm, mean.mspe.lrna2.svm + 1.96 * sqrt(sigma.mspe.lrna2.svm))
#testlm <- lm(test.lrna2$y~ pred.svm.vec - 1)
var(test.lrna2$y)
ggplot(data.frame(cbind(pred.svm.vec,test.lrna2$y)), aes(x = pred.svm.vec, y = test.lrna2$y))+
  geom_point() + ylim(-4,4) + xlim(-1.5,1.5) +geom_abline(intercept = 0, slope = 1)+
  labs(title = 'lrna2 in test set vs predicted value - RPART-SVM',x = 'Predicted value',y='True test value')
cor(pred.svm.vec,test.lrna2$y) #-0.01614301 not norm
R2.rpart <- 1 - sum((test.lrna2$y-pred.svm.vec)^2)/sum((test.lrna2$y-mean(test.lrna2$y))^2)
R2.rpart #-0.0916268 not norm

#TEST ERROR [1] 0.8659616 1.0441924 1.2224231
# 0.8566347 1.1423722 1.4281097 not norm
